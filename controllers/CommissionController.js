...
const deductSlot = async(req, res, next) => {
    try {
        let slot = {
            slot: req.body.slot
        }
        //finds user, then commission service id from the database to be deleted 
        Commission.findByIdAndUpdate(req.body._id, slot)
            .then((result) => {
                res.status(200).json({
                    message: 'Commission slot updated successfully!',
...
module.exports = { 
    addCommission, getCommissionList, updateCommission,deleteCommission,deductSlot
}